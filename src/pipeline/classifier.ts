...
// Light depth — single complete module or task with multiple sub-requirements
const LIGHT_ZH = /设计.{0,15}(API|接口|数据库|表结构|方案|架构|组件|页面|模块)|完整.{0,10}(组件|模块|页面|接口|API|类|服务)|实现.{0,15}(完整|整个|全部).{0,10}(功能|模块|组件|页面)/;
const LIGHT_EN = /design (?:a |an |the )?(?:complete |full )?.{0,20}(?:API|component|module|page|schema|service|class|interface)|(?:full|complete|entire) .{0,20}(?:component|module|page|implementation|service|class)|implement (?:a |an |the )?(?:complete |full |entire ).{0,20}(?:feature|module|component|page)/i;
// Multi-component tasks — listing multiple sub-items indicates non-trivial complexity
const MULTI_COMPONENT_ZH = /(?:包括|包含|涵盖|需要|实现).{0,10}(?:、|，.{1,8}(?:、|和|及))/;
const MULTI_COMPONENT_EN = /(?:including|with|containing|implementing) .{1,30}(?:,| and ) .{1,30}(?:,| and )/i;
...
export function classifyDepthFastPath(userRequest: string): PipelineDepth | null {
  ...
  if (LIGHT_ZH.test(userRequest) || LIGHT_EN.test(userRequest)) return 'light';
  ...
  if (
    ...
    // Multi-component tasks list multiple requirements — light depth
    if (MULTI_COMPONENT_ZH.test(userRequest) || MULTI_COMPONENT_EN.test(userRequest)) {
      return 'light';
    }
    return 'direct';
  }
