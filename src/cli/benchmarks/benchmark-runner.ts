...
export async function runBenchmarkSuite(
  ...
  const probeEM = new EndpointManager();
  probeEM.setEndpoints(allEndpoints);
  console.log(chalk.dim(`  Probing ${allEndpoints.length} endpoints...`));

  const probeModel = config.compressor.model;
  const sharedWorkingEndpoints = await probeEM.probeAllEndpoints(probeModel);
