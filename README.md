...
### 📊 v0.1.2 基准测试（2026-03-31）

**Baseline 对比**（gpt-5.4 强模型 vs NTK 全廉价模型路由）：

| 任务 | Strong 直接 | NTK | Token 变化 | 强模型用量 |
|------|-----------|-----|-----------|----------|
| 简单代码生成 | 173 tok | 549 tok | +217% | **0（全廉价）** |
| 技术对比（中等） | 1164 tok | **590 tok** | **-49%** | **0** |
| API 设计（中等） | 2489 tok | **1584 tok** | **-36%** | **0** |
| Debug 分析 | 377 tok | 505 tok | +34% | **0** |

**三方对比**（11 个测试用例，传统版 vs 类 RTK 版 vs NTK）：

| 方案 | 总 Token | 加权成本节省 |
|------|----------|-----------|
| Traditional（全强模型） | 15,437 | 基准 |
| Filter-only（确定性过滤） | 13,894 | ~10% |
| **NTK** | **11,991** | **~92%** |

**关键优势**：中等/复杂任务 NTK 用更少的 token + 100% 廉价模型 = 综合成本节省 **90%+**。

**v0.1.2 新优化**：智能验证跳过使 light 深度 token 减少 **76%**（6528→1584），耗时减少 **65%**。
...
> I/O 过滤压缩的是数据（删字符），NTK 压缩的是认知（控信息流）。NTK 兼容所有 I/O 层过滤能力，同时额外拥有语义压缩、路由隔离、自适应深度等独有能力。
